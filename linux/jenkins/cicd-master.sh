...
----------------------------------\n"
        ...
        if [[ "${DeployENV}" == "${ENV_domain}" ]] && [[ "${PublishEnvironment}" == "${ENV_FAT}" || "${PublishEnvironment}" == "${ENV_UAT}" ]] && [[ "${BuildOrCopy}" == "${ACTION_COPY}" ]]; then
                log_fail "${ENV_domain} only allow ${ENV_PRO}/${ENV_PREPRO} environment "${ACTION_COPY}"!"
                exit 10
        fi
...
git_k8s_clone_for_copy(){
	...
	if [ ${GIT_K8S_ZONE_ENV} == "${ENV_domain}" ] && [ ${GIT_K8S_PROJECT_ENV} == "${ENV_PRO}" ];then
	...
	elif [ ${GIT_K8S_ZONE_ENV} == "${ENV_domain}" ] && [ ${GIT_K8S_PROJECT_ENV}  == "${ENV_PREPRO}" ];then
		log_info "Clone -b ${PEER_ENV} ${GIT_K8S_PROJECT_FULLNAME}"
		sudo rm -rf ${LANGUAGE}-${GIT_K8S_PROJECT_NAME} && \
		sudo git clone -b ${PEER_ENV} ${GIT_K8S_PROJECT_FULLNAME} 
	fi
...
k8s_deploy_for_copy(){
        if [ ${GIT_K8S_ZONE_ENV} == "${ENV_domain}" ];then
		# get image version
                git_k8s_clone_for_copy
                git_k8s_get_image_version_for_copy && \
...
git_k8s_clone(){
	...
	if [ ${GIT_K8S_ZONE_ENV} == "${ENV_domain}" ] && [ ${GIT_K8S_PROJECT_ENV} == "${ENV_PRO}" -o ${GIT_K8S_PROJECT_ENV} == "${ENV_PREPRO}" ];then
	...
	elif [ ${GIT_K8S_ZONE_ENV} == "${ENV_domain}" ] && [ ${GIT_K8S_PROJECT_ENV} == "${ENV_FAT}" -o ${GIT_K8S_PROJECT_ENV} == "${ENV_UAT}" ];then
...
git_k8s_imageUpdate(){
	...
	sudo git config --global user.email "Jenkins@domain.com"
...
main(){
        ...
        #if ([[ "${DeployENV}" == "${ENV_domain}" ]] && [[ "${PublishEnvironment}" == "${ENV_FAT}" || "${PublishEnvironment}" == "${ENV_UAT}" ]]) || \
        #([[ "${DeployENV}" == "${ENV_domain}" ]] && [[ "${PublishEnvironment}" == "${ENV_PREPRO}" || "${PublishEnvironment}" == "${ENV_PRO}" ]] && [[ "${BuildOrCopy}" == "${ACTION_BUILD}" ]]);then
        if [[ "${DeployENV}" == "${ENV_domain}" ]] && [[ "${PublishEnvironment}" == "${ENV_FAT}" || "${PublishEnvironment}" == "${ENV_UAT}" || "${PublishEnvironment}" == "${ENV_PREPRO}" || "${PublishEnvironment}" == "${ENV_PRO}" ]] && [[ "${BuildOrCopy}" == "${ACTION_BUILD}" ]];then
                compile
                docker_build
                docker_tag_image
                docker_login
