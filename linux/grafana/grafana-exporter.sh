...
counter=0
for dashboard_uid in $(curl -sS -H "Authorization: Bearer $KEY" $PT$HOST/api/search\?query\=\& | jq -r '.[] | select( .type | contains("dash-db")) | .uid'); do
  
   counter=$((counter + 1))
   url=`echo $PT$HOST/api/dashboards/uid/$dashboard_uid | tr -d '\r'`
