...
function export(){
	RES_JSON=`curl -sS -X POST -H "Content-Type: application/json" -d '{"name":"'${APIKEY_NAME}'", "role": "Viewer", "secondsToLive": '${APIKEY_TIME}'}' $PROTOCOL${USER}:${PASSWORD}@${HOST}/api/auth/keys`
	APIKEY=`echo $RES_JSON | jq -r '.key'`
	cd ${BACKUP_DIR} 
	./grafana-exporter.sh ${HOST} "$APIKEY" ${PROTOCOL}
	./grafana-exporter-raw.sh ${HOST} "$APIKEY" ${PROTOCOL}
}

function delete_apikey(){
	GET_RES_JSON=`curl -sS -X GET $PROTOCOL${USER}:${PASSWORD}@${HOST}/api/auth/keys?name=${APIKEY_NAME}`
	...
	DELETE_RES_JSON=`curl -sS -X DELETE $PROTOCOL${USER}:${PASSWORD}@${HOST}/api/auth/keys/${APIKEY_ID}`
